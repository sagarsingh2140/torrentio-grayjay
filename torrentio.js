...
function buildConfig() {
  var p = (_settings.debridProvider || "none").trim();
  var k = (_settings.debridApiKey || "").trim();
  var parts = ["sort=quality", "limit=3"];
  if (p !== "none" && k.length > 0) {
    parts.push(p + "=" + k);
  }
  return parts.join("|");
}
...
function getStreams(id, s, e) {
  try {
    var cfg = buildConfig();
    var path;
    if (s != null) {
      path = "/stream/series/" + id + ":" + s + ":" + e + ".json";
    } else {
      path = "/stream/movie/" + id + ".json";
    }
...
function toVideo(meta, type) {
  ...
  var year = meta.year ? parseInt(String(meta.year)) : 0;
...
source.search = function(query) {
  ...
  var movies = cinemetaSearch(query, "movie").map(function(m) {
    return toVideo(m, "movie");
  });
  var series = cinemetaSearch(query, "series").map(function(m) {
    return toVideo(m, "series");
  });
  var merged = [];
...
source.getVideoDetails = function(url) {
  ...
  try {
    ...
    if (r.isOk) {
      meta = JSON.parse(r.body).meta || {};
    }
  } catch(e) {}
  var raw = type === "movie"
    ? getStreams(imdbId, null, null)
    : getStreams(imdbId, 1, 1);
